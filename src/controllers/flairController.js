...
module.exports = {
  ...
  destroy(req, res, next) {
    flairQueries.deleteFlair(req.params.id, (err, deletedFlairsCount) => {
      if (err) {
        res.redirect(
          500,
          `/topics/${req.params.topicId}/flairs/${req.params.id}`
        );
      } else {
  ...
  update(req, res, next) {
    flairQueries.updateFlair(req.params.id, req.body, (err, flair) => {
      if (err || flair == null) {
        res.redirect(
          404,
          `topics/${req.params.topicId}/flairs/${req.params.id}/edit`
        );
      } else {
